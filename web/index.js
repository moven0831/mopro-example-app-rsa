...
import rsaInput from '../test-vectors/noir/rsa_input.json';
...
//   // Path relative to web/index.js: ../test-vectors/noir/zkemail_srs.local
//   const response = await fetch('../test-vectors/noir/zkemail_srs.local');
//   if (!response.ok) {
...
// Function to fetch the zkemail input JSON
async function getZkEmailInput() {
  showAppend("logs", "Fetching zkemail input JSON... ⏳");
  // Path relative to web/index.js: ../test-vectors/noir/zkemail_input.json
  // const response = await fetch('../test-vectors/noir/zkemail_input.json');
  // if (!response.ok) {
  //   showAppend("logs", `Failed to fetch zkemail input JSON: ${response.statusText} 💔`);
  //   throw new Error(`Failed to fetch zkemail input JSON: ${response.statusText}`);
  // }
  // const zkemailInput = await response.json();
  showAppend("logs", "Fetched zkemail input JSON... ✅");
  return rsaInput;
}
...
document.getElementById("submit").addEventListener("click", async () => {
  try {
    ...
    const inputs = await getZkEmailInput();
